...
namespace MCLauncher {
    ...
    public partial class MainWindow : Window, ICommonVersionCommands {
        ...
        private void BackupMinecraftDataForRemoval(out string backupPath, out string targetPath) {
            ...
            backupPath = tmpDir;
            targetPath = data.LocalFolder.Path;
            Debug.WriteLine("Moving Minecraft data to: " + backupPath);
            Directory.Move(data.LocalFolder.Path, backupPath);
        }

        private void RestoreMinecraftDataAfterRemoval(string backupPath, string targetPath) {
            if (!Directory.Exists(backupPath))
                return;
            Debug.WriteLine("Moving backup Minecraft data to: " + targetPath);
        ...
        private async Task RemovePackage(Package pkg) {
            ...
            if (!pkg.IsDevelopmentMode) {
                string backupPath, targetPath;
                BackupMinecraftDataForRemoval(out backupPath, out targetPath);
                await DeploymentProgressWrapper(new PackageManager().RemovePackageAsync(pkg.Id.FullName, 0));
                RestoreMinecraftDataAfterRemoval(backupPath, targetPath);
            } else {
